...
if(isset($_POST["submit_ins"])){
	...
	if(empty($temid)){
		$sub = $_POST["mail_sub"];
		$temp = $_POST["temp_desc"];
	}else{
		$sql="SELECT * FROM email_template where id = '$temid'";
		$result= $dbObj->select($sql);

		$sub = @$result[0]['tem_name'];
		$temp = @$result[0]['tem_desc'];
	}
	
 foreach ($_POST['check'] as $ac_code){
...
$pdf = new FPDF('p','mm','A4');
		...
		$sum=0;
		$no_of_share=0;
	    $sql_ipo="SELECT * FROM ipo_application WHERE ac_no='$ac_code' AND status=1 AND process_status=0";
...
$sql2l="SELECT * FROM `loan_catagory_margin` WHERE id='$loan_cat'";
	 ...
	 $pdf->Ln(10);
		...
		$pdf->Ln(3);
		$pdf->SetFont('Helvetica','B',8);
		$pdf->Write(4,'Instrument                               Effective/Applied              Declare On              Holding                  Quantity             Rate                 Total Amount');
		$pdf->Ln(5);
		  $d_sum=0;
		 for($i=0; $i<count($bonus_share);$i++){
		$pdf->SetFont('Helvetica','',8);  
		$pdf->Write(4, @$bonus_share[$i]['isin_short_name']."                ".@$bonus_share[$i]['effective_date']."                  ".$bonus_share[$i]['record_date']."                   ".$bonus_share[$i]['bo_holding']."                       ".$bonus_share[$i]['qty']."                   ".@$bonus_share[$i]['market_price']."                    ".@$bonus_share[$i]['qty']*@$bonus_share[$i]['market_price']." ");
		$pdf->Ln(12);
	}
		...
		$pdf->SetFont('Helvetica','',8);  
		$pdf->Write(4, @$res_ipo[$i]['company_id']."                   ".$applied_date."                      ".$declaration_date."                   ".'0'."                                 ".$res_ipo[$i]['market_lot']."                        ".@$res_ipo[$i]['ipo_name']."                  ".@$res_ipo[$i]['total_amt']." ");
		$pdf->Ln(12);
		}
	}
	...
	$pdf->SetFont('Helvetica','',9);
		...
		$pdf->Ln(3);
		$pdf->SetFont('Helvetica','B',8);
		$pdf->Write(4,' Instrument                       Cash Payment Date                          Record Date                   Holding                          Net Cash Amount        ');
		$pdf->Ln(5);
		  $total_cash=0;
		...
		$pdf->SetFont('Helvetica','',8);  
		$pdf->Write(4, $cash_dividend[$i]['isin_short_name']."                   ".@$cash_dividend[$i]['payment_date']."                          ".$cash_dividend[$i]['record_date']."                   ".$cash_dividend[$i]['bo_holding']."                        ".@$cash_dividend[$i]['net_cash']." ");
		$pdf->Ln(5);
	}
	 $pdf->SetFont('Helvetica','B',10);
	 $pdf->Write(1, '                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   ---------------------------------------------------------');
	 $pdf->Ln();
	 $pdf->Write(4, "                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   Total :                               $total_cash" );
	 $pdf->Ln();
}
        $pdf->SetFont('Arial','',8);
      ...
      $fileName ='investor_protfolio_'.$currDate.'_'.$ac_code.'.pdf';
      $pdf->Output('portpolio_report/'.$fileName, 'F');
		...
		//$mail->AddCC("farzana@capmadvisorybd.com");
