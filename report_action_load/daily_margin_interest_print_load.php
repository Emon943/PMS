...
if($ac){
   $acc_no=$ac;
  $vvv="SELECT * FROM `tbl_interest` WHERE `code`='mi' AND `ac_no`='$ac' AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY `date` ASc";
  
  $res= $dbObj->select($vvv);
  //print_r($res);
 
  $sql1="SELECT SUM(daily_interest)as total_interest_amt FROM `tbl_interest` where code='mi' AND `ac_no`='$ac' AND `date` BETWEEN '$fromdate' AND '$todate'";
  $result= $dbObj->select($sql1);
  $total=$result[0]['total_interest_amt'];
}
else{
   $acc_no='All';
  $sql="SELECT * FROM `tbl_interest` where code='mi' AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY date";
  $res= $dbObj->select($sql);
  $sql1="SELECT SUM(daily_interest)as total_interest_amt FROM `tbl_interest` where code='mi' AND `date` BETWEEN '$fromdate' AND '$todate'";
  $result= $dbObj->select($sql1);
  //print_r($result);
  $total=$result[0]['total_interest_amt'];
  //print_r($res);
...
<br>
	...
	<h5 style="float:right;font-size: 12px; font-weight:bold;">Daily Margin Interest</h5>
	</div>
...
title = 'Daily Margin Interest Report' + '\n' +'-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------';
	$(document).ready(function() {
    $('#example').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Daily margin interest',  messageTop: "<?php echo "From Date: ".$fromdate."          "."To Date: ".$todate."          "."A/C NO: ".$acc_no."                                            "."Report Date: ".date("d-M-Y"); ?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Daily margin interest',  messageTop: "<?php echo "From Date: ".$fromdate."          "."To Date: ".$todate."          "."A/C NO: ".$acc_no."                                               "."Report Date: ".date("d-M-Y"); ?>",
