<?php
 ...
 $to_date = date('m-d-Y', strtotime($todate));
  ...
  $sql_e="SELECT investor_name FROM `investor` where dp_internal_ref_number='$ac_no'";
  $resul= $dbObj->select($sql_e);
  $investor_name=$resul[0]['investor_name'];
 ...
 //die();
  ...
  $sql="SELECT * FROM tbl_charge_info WHERE ac_no='$ac_no' AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY date ASC";
   $ledger_res= $dbObj->select($sql);
  // print_r($ledger_res);
   //die();
?>
  ...
  <?php 
 if($ledger_res || $balace_forward){

 ?>
			        <br>
                   	<div style="border-bottom: 2px solid; border-bottom-style:solid;">
					<h5 style="float:right;font-size: 12px; font-weight:bold;">Client Ledger Statement</h5>
					</div>
					...
					<div>
					<h5><strong>Investor Name : </strong><?php echo $investor_name;?></h5>
					</div>
	...
	<tbody>
	 ...
	 $sum=$sum+$bf;
	  $dr=0.00;
	   $cr=0.00;
	 foreach($ledger_res as $r){
		  $code=$r['code'];
		 if($r['type']=='Buy' || $r['type']=='Payment'){
			 $debit_amt=$r['total_amt'];
			 $dr=$dr+$debit_amt;
		}else{
			 $credit_amt=$r['total_amt'];
			  $cr=$cr+$credit_amt;
		}
		...
		?>
                                    ...
                                    <tr>
                                        ...
                                        <th  scope="row"><?php echo date('d-m-Y', strtotime($r['date']));?></th>
										<td><?php echo $ac_no;?></td>
                                        <td><?php echo $r['type'];?></td>
										<td><?php if($code=="RF" || $code=="ISC"){
										echo $r['description']." ".$r['short_name'];
	                                 }else{
										 echo $r['description'];
									 }
									 ?>
										...
										<td><?php echo $r['commi'];?></td>
                
										<td>
										<?php if($r['type']=='Buy' || $r['type']=='Payment'){
										   echo $r['total_amt'];
										   $sum=$sum-$r['total_amt'];
										}else{
										 echo "0.00";	
										...
										<td>
										<?php if($r['type']=='Sale' || $r['type']=='Receipt'){
										   echo $r['total_amt'];
										    $sum=$sum+$r['total_amt'];
										}else{
										 echo "0.00";	
			...
			<td></td>
			<td></td>
			<td><span style="font-weight:bold">Total Dr. <?php echo number_format((float)$dr, 2); ?></span></td>
			<td><span style="font-weight:bold">Total Cr. <?php echo number_format((float)$cr, 2); ?></span></td>
            <td><span style="font-weight:bold">Total Bal <?php echo number_format((float)$sum, 2); ?></span></td>
...
var title = 'CAPM Advisory Limited' + '\n' + 'Tower Hamlet(9th floor),' + '\n' + '16 Kamal Ataturk Avenue,Banani C/A'+ '\n' +'Dhaka-1213,Bangladesh.'+ '\n' + 'Client Ledger Statement';
    ...
    $('#example').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            { extend: 'print', footer: true, title: title, filename: 'Client Ledger Statement',  messageTop: "<?php echo"From Date: ".$fromdate."          "."TO Date: ".$todate."          "."Investor Name: ".$investor_name."      "."Report Date: ".date("d-M-Y")."      "."Balance Forward : ".$bf; ?>" },
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Client Ledger Statement',  messageTop: "<?php echo"To Date: ".$fromdate."          "."TO Date: ".$todate."          "."Investor Name: ".$investor_name."      "."Report Date: ".date("d-M-Y")."      "."Balance Forward : ".$bf; ?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Client Ledger Statement',  messageTop: "<?php echo"From Date: ".$fromdate."          "."TO Date: ".$todate."          "."Investor Name: ".$investor_name."      "."Report Date: ".date("d-M-Y")."      "."Balance Forward : ".$bf; ?>",
