...
if($ac){
  ...
  $vvv="SELECT * FROM `tbl_interest_on_credit_bal` WHERE `ac_no`='$ac' AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY `date`";
  
  $res= $dbObj->select($vvv);
  //print_r($res);
 
  $sql1="SELECT SUM(daily_interest)as total_interest_amt,SUM(acmf_interest_amt)as total_acmf_amt FROM `tbl_interest_on_credit_bal` where `ac_no`='$ac' AND `date` BETWEEN '$fromdate' AND '$todate'";
  $result= $dbObj->select($sql1);
  $total=$result[0]['total_interest_amt'];
  $total_acmf=$result[0]['total_acmf_amt'];
}
else{
  
  $sql="SELECT * FROM `tbl_interest_on_credit_bal` where `date` BETWEEN '$fromdate' AND '$todate'";
  $res= $dbObj->select($sql);
  $sql1="SELECT SUM(daily_interest)as total_interest_amt,SUM(acmf_interest_amt)as total_acmf_amt FROM `tbl_interest_on_credit_bal` where `date` BETWEEN '$fromdate' AND '$todate'";
  $result= $dbObj->select($sql1);
  //print_r($result);
  $total=$result[0]['total_interest_amt'];
  $total_acmf=$result[0]['total_acmf_amt'];
...
?>
 ...
 ?>
	...
	<h5 style="float:right;font-size: 12px; font-weight:bold;">Daily cash management fee</h5>
	</div>
    ...
    <tr>
	  ...
	  <th scope="col">Acc Mgt. Rate(%)</th>
	  <th scope="col">Acc Mgt. Amount</th>
	   <th scope="col">Status</th>
                            ...
                            <tbody>
                               ...
                               <?php foreach($res as $r){
								 $ac=$r['ac_no'];
								 $sql_e="SELECT investor_name FROM `investor` where dp_internal_ref_number='$ac'";
							   ...
							   ?>
                                    ...
                                    <tr>
										...
										<td><?php echo $r['acmf_interest_amt'];?></td>
										<td>
										<?php if($r['status']==0){
										echo "Accrued";
										}else{
			...
			<td></td>
            <td><span style="font-weight:bold"><?php echo $total_acmf; ?></span></td>
			<td></td>
            </tr>
        </tfoot>
...
var title = 'CAPM Advisory Limited' + '\n' + 'Tower Hamlet(9th floor),' + '\n' + '16 Kamal Ataturk Avenue,Banani C/A'+ '\n' +'Dhaka-1213,Bangladesh.'+ '\n' + 'Daily cash management fee report';
	$(document).ready(function() {
    $('#example').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Daily cash management fee',  messageTop: "<?php echo"To Date: ".$fromdate."          "."TO Date: ".$todate."          "."A/C NO: ".$ac."      "."Report Date: ".date("d-M-Y"); ?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Daily cash management fee',  messageTop: "<?php echo"From Date: ".$fromdate."          "."TO Date: ".$todate."          "."A/C NO: ".$ac."      "."Report Date: ".date("d-M-Y"); ?>",
