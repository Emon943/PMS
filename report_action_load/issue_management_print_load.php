<?php
 include("../config.php");
 $dbObj = new DBUtility();
 
$fromdate=@$_POST["fromdate"];
$previous_date=date('Y-m-d', strtotime('-1 day', strtotime($fromdate)));
$start_date = date('M-Y', strtotime($fromdate));
$todate=@$_POST["todate"];
          $sql="SELECT count(id) as margin_code FROM loan_catagory_margin
           INNER JOIN investor_group ON loan_catagory_margin.id = investor_group.loan_cat
           INNER JOIN investor ON investor.investor_group_id = investor_group.investor_group_id WHERE investor.status=0";
		  $result= $dbObj->select($sql);
		  $margin_code=$result[0]['margin_code'];
		  
		  
		  $sqltop="SELECT SUM(balace_forward) as ledger_bal FROM tbl_balance_forward WHERE `date`='$previous_date'";
          $total_opening_bal= $dbObj->select($sqltop);
		  $total_open_bal=$total_opening_bal[0]['ledger_bal'];
		  
		  $sqltc="SELECT SUM(balace_forward) as ledger_bal FROM tbl_balance_forward WHERE `date`='$todate'";
          $total_closing_bal= $dbObj->select($sqltc);
		  $total_closing_bal=$total_closing_bal[0]['ledger_bal'];
		  
		  
          $sql1="SELECT count(dp_internal_ref_number) as total_code FROM investor WHERE investor.status=0";
		  $res= $dbObj->select($sql1);
		  $total_code=$res[0]['total_code'];
		  $non_margin_code=$total_code-$margin_code;
		  // echo "<pre>";
		  //print_r($res);
		  // echo "</pre>";
		  
		  //echo "<pre>";
		  //print_r($result);
		  // echo "</pre>";
		  //die();
		  $sqlb="SELECT SUM(total_amt) as buy_amt FROM tbl_charge_info WHERE code='B' AND `date` BETWEEN '$fromdate' AND '$todate'";
          $buy= $dbObj->select($sqlb);
		  $buy_amt=$buy[0]['buy_amt'];
		  $sqls="SELECT SUM(total_amt) as sell_amt FROM tbl_charge_info WHERE code='S' AND `date` BETWEEN '$fromdate' AND '$todate'";
          $sell= $dbObj->select($sqls);
		  $sell_amt=$sell[0]['sell_amt'];
		   
          $sqlmc="SELECT investor.dp_internal_ref_number,investor.total_balance,loan_catagory_margin.interest_rate,investor_group.loan_applicable
           FROM loan_catagory_margin
           INNER JOIN investor_group ON loan_catagory_margin.id = investor_group.loan_cat
           INNER JOIN investor ON investor.investor_group_id = investor_group.investor_group_id WHERE investor.status=0";
		  $mc_res= $dbObj->select($sqlmc);
		   //echo "<pre>";
		  //print_r($mc_res);
		   //echo "</pre>";
		  $bsum=0.00;
		  $ssum=0.00;
		  $osum=0.00;
		  $csum=0.00;
		  
		  for($i=0; $i< count($mc_res); $i++){
		  $ac=@$mc_res[$i]['dp_internal_ref_number'];
		  $sqlbb="SELECT SUM(total_amt) as buy_amount FROM tbl_charge_info WHERE code='B' AND ac_no='$ac' AND `date` BETWEEN '$fromdate' AND '$todate'";
          $buy_res= $dbObj->select($sqlbb);
		  $buy_amount=$buy_res[0]['buy_amount'];
		  $bsum=$bsum+$buy_amount;
		  $sqlss="SELECT SUM(total_amt) as sell_amount FROM tbl_charge_info WHERE code='S' AND ac_no='$ac' AND `date` BETWEEN '$fromdate' AND '$todate'";
          $sell_res= $dbObj->select($sqlss);
		  $sell_amount=$sell_res[0]['sell_amount'];
		  $ssum=$ssum+$sell_amount;
		  
		  $sqlop="SELECT SUM(balace_forward) as ledger_bal FROM tbl_balance_forward WHERE ac_no='$ac' AND `date`='$previous_date'";
          $opening_bal= $dbObj->select($sqlop);
		  $open_bal=$opening_bal[0]['ledger_bal'];
		  $osum=$osum+$open_bal;
		  $sqlcl="SELECT SUM(balace_forward) as ledger_bal FROM tbl_balance_forward WHERE ac_no='$ac' AND `date`='$todate'";
          $closing_bal= $dbObj->select($sqlcl);
		  $close_bal=$closing_bal[0]['ledger_bal'];
		  $csum=$csum+$close_bal;
		  }
		   //echo "buy".$bsum.'<br>';
		   //echo "sell".$ssum.'<br>';
		   $loan_disbursed=$osum-$csum;
		  $non_margin_buy_amt=$buy_amt-$bsum;
		  $non_margin_sell_amt=$sell_amt-$ssum;

          //echo "<pre>";
		 // print_r($sell);
		   //echo "</pre>";
?>

    <br>
	<div style="border-bottom: 2px solid; border-bottom-style:solid;">
	<h5 style="float:right;font-size: 12px; font-weight:bold;">Issue Management Report</h5>
	</div>
	<div style="font-size: 12px; font-weight:bold; margin-bottom: 2px;">
		<!--        <h6 class="report_title" style="text-align: center; padding: 10px;">Admit Card</h6>-->
		<!--<h6>Student ID: <?php /*echo $student['TBL35_REGISTRATION_NUMBER'];*/?></h6>-->
	<p style="font-size: 12px; font-weight:bold;">FROM Date: <?php echo $fromdate; ?>     &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;   To Date: <?php echo $todate;?></p>
 
	</div>
					
   <table id="example" class="display" style="width:100%;">
    <thead>
    <tr>
      <th scope="col"> </th>
      <th scope="col"> </th>
      
    </tr>
	
    <tbody>
		<tr>
		 <td>BO Accounts Type:</td>
		 <td> Non-discretionary</td>
	   </tr>
        <tr>
		 <td>No. of BO Accounts:</td>
		 <td><?php echo $non_margin_code;?></td>
	    </tr>
        <tr>
		 <td>Total Buy (Customer A/C):</td>
		 <td><?php echo number_format((float)$non_margin_buy_amt, 2);?></td>
	    </tr>
        <tr>
		 <td>Total Sale (Customer A/C):</td>
		 <td><?php echo number_format((float)$non_margin_sell_amt, 2);?></td>
	    </tr>
        <tr>
		 <td>Margin Loan Opening Balance:</td>
		 <td><?php echo number_format((float)$total_opening_bal, 2);?></td>
	    </tr>
      <tr>
		 <td>Margin Loan Disbursed:</td>
		 <td><?php echo number_format((float)$non_margin_sell_amt, 2);?></td>
	  </tr>
       <tr>
		 <td>Margin Loan Realized:</td>
		 <td>0.00</td>
	   </tr>
       <tr>
		 <td>Margin Loan Closing Balance:</td>
		 <td><?php echo number_format((float)$total_closing_bal, 2);?></td>
	  </tr>
 	  <tr>
		 <td></td>
		 <td></td>
	  </tr>	
	  <tr>
		 <td>BO Accounts Type:</td>
		 <td> Discretionary </td>
	  </tr>	
	  <tr>
		 <td>No. of BO Accounts:</td>
		 <td><?php echo $margin_code;?></td>
	  </tr>
       <tr>
		 <td>Total Buy (Customer A/C):</td>
		 <td><?php echo number_format((float)$bsum, 2);?></td>
	  </tr>
      <tr>
		 <td>Total Sale (Customer A/C):</td>
		 <td><?php echo number_format((float)$ssum, 2);?></td>
	  </tr>
      <tr>
		 <td>Margin Loan Opening Balance:</td>
		 <td>-</td>
	  </tr>	
      <tr>
		 <td>Margin Loan Disbursed:</td>
		 <td>-</td>
	  </tr>	
      <tr>
		 <td>Margin Loan Realized:</td>
		 <td>-</td>
	  </tr>
      <tr>
		 <td>Margin Loan Closing Balance:</td>
		 <td>-</td>
	  </tr>		  
         
    </tbody>
					   
   </table>
						
            
		
<script>
messageBottom='Tower Hamlet (9th Floor), Kemal Ataturk Avenue, Banani C/A, Dhaka-1213,Bangladesh' + '\n' + ' Contact us | Tel: 02-9822391-2 | Fax: 02-9822393 | Web: www.capmadvisorybd.com';
var today = new Date();
var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
//var title = 'CAPM Advisory Limited' + '\n' + 'Tower Hamlet(9th floor),' + '\n' + '16 Kamal Ataturk Avenue,Banani C/A'+ '\n' +'Dhaka-1213,Bangladesh.'+ '\n\n' + 'Daily Margin Interest Report';
title = 'Issue Management Report Form' + '\n' +'-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------';
	$(document).ready(function() {
    $('#example').DataTable( {
		"order": [],
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Issue Management',  messageTop: "<?php echo "Date: ".$start_date."                                        "; ?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Issue Management',  messageTop: "<?php echo "Date: ".$start_date."                                 "; ?>",
			messageBottom: messageBottom,
			pageSize: 'A4',
			
			customize: function ( doc ) {
				
				doc.pageMargins = [10,10,10,10];
                doc.defaultStyle.fontSize = 8;
                 doc.styles.tableHeader.fontSize = 11;
                 //doc.styles.title.fontSize = 7;
                    doc.content.splice( 0, 0, {
						width:120,
						height:60,
					    image: 'data:image/png;base64,'
						
                    } )
					doc.styles.title = {
				    //color: 'red',
				    fontSize: '9',
				    alignment: 'right',
					italics: 'true'
				   } 
				   
doc['footer']=(function(page, pages) {
return {
columns: [
'print date: '+date,
{
alignment: 'right',
text: [
'PageNo  ',
{ text: page.toString(), italics: true },
' of ',
{ text: pages.toString(), italics: true }
]
}
],
margin: [30, 0, 30, 10]
}
});

				   
                }
			
			}
        ]
		
    } );
	
} );


</script>


