...
include("../config.php");
 ...
 $sqls="SELECT SUM(balace_forward) as lb,SUM(ava_balance) as ab FROM tbl_balance_forward where date='$as_on' ORDER BY ac_no ASC";
 $res_sum= $dbObj->select($sqls);
 $ledger_balance=$res_sum[0]['lb'];
 $avi_balance=$res_sum[0]['ab'];
 $acc="All";
}else{
 $sql="SELECT * FROM tbl_balance_forward where date='$as_on' AND ac_no='$ac'";
 $result= $dbObj->select($sql);
  $sqls="SELECT SUM(balace_forward) as lb,SUM(ava_balance) as ab FROM tbl_balance_forward where date='$as_on' AND ac_no='$ac'";
 $res_sum= $dbObj->select($sqls);
 $ledger_balance=$res_sum[0]['lb'];
 $avi_balance=$res_sum[0]['ab'];
 $acc=$ac;
...
<style>
 ...
 ?>
					...
					<h5 style="float:right;font-size: 12px; font-weight:bold;">Client Account Status Details</h5>
					</div>
					...
					<p style="font-size: 12px; font-weight:bold;">Date: <?php echo $as_on; ?> </p>
                 
                    </div>
					
					<div>
					<h5><strong>Investor Name : </strong><?php echo $acc;?></h5>
					</div>
    ...
    <tr>
      ...
      <th scope="col">Immatured Sale</th>
      <th scope="col">Unclear Cheque</th>
	  ...
	  if($status=="0"){
		...
		?>
                                    ...
                                    <tr>
                                        ...
                                        <th><?php echo $ac_no;?></th>
										 <th><?php echo $investor_name;?></th>
										   <th><?php echo number_format((float)$immatured, 2);?></th>
										     <th><?php echo $unclear_cheque;?></th>
											   <th><?php echo number_format((float)$ava_balance, 2);?></th>
											     <th><?php echo number_format((float)$balace_forward, 2);?></th>
												   <th><?php echo $a;?></th>
			...
			<td></td>
			<td></td>
			<td><?php echo number_format((float)$avi_balance, 2); ?></td>
            <td><?php echo number_format((float)$ledger_balance, 2); ?></td>
...
var title = 'CAPM Advisory Limited' + '\n' + 'Tower Hamlet(9th floor),' + '\n' + '16 Kamal Ataturk Avenue,Banani C/A'+ '\n' +'Dhaka-1213,Bangladesh.'+ '\n\n' + 'Client Account Status Details' + '\n' +'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------';
	$(document).ready(function() {
    $('#example').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            { extend: 'print', footer: true, title: title, filename: 'Client Account Status Details',  messageTop: "<?php echo"Date: ".$as_on."          "."          "?>" },
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Client Account Status Details',  messageTop: "<?php echo"Date: ".$as_on."          "."         "?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Client Account Status Details',
           orientation: 'portrait',
			pageSize: 'A4',
			messageTop:'Client Code :  <?php echo  $ac_no ?>                                                                                                                                                         Date :  <?php echo $as_on; ?>\n\n Client Code :  <?php echo $investor_name;?> ',
