...
if($ac){
  ...
  $vvv="SELECT * FROM `tbl_interest` WHERE `code`='mf' AND `ac_no`='$ac' AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY `date` ASc";
  
  $res= $dbObj->select($vvv);
  //print_r($res);
 
  $sql1="SELECT SUM(daily_interest)as total_interest_amt FROM `tbl_interest` where code='mf' AND `ac_no`='$ac' AND `date` BETWEEN '$fromdate' AND '$todate'";
  $result= $dbObj->select($sql1);
  $total=$result[0]['total_interest_amt'];
}
else{
  
  $sql="SELECT * FROM `tbl_interest` where code='mf' AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY date";
  $res= $dbObj->select($sql);
  $sql1="SELECT SUM(daily_interest)as total_interest_amt FROM `tbl_interest` where code='mf' AND `date` BETWEEN '$fromdate' AND '$todate'";
  $result= $dbObj->select($sql1);
  //print_r($result);
  $total=$result[0]['total_interest_amt'];
  //print_r($res);
...
<br>
	...
	<h5 style="float:right;font-size: 12px; font-weight:bold;">Daily management fee</h5>
	</div>
...
var title = 'CAPM Advisory Limited' + '\n' + 'Tower Hamlet(9th floor),' + '\n' + '16 Kamal Ataturk Avenue,Banani C/A'+ '\n' +'Dhaka-1213,Bangladesh.'+ '\n' + 'Daily management fee report';
	$(document).ready(function() {
    $('#example').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Daily management fee',  messageTop: "<?php echo"To Date: ".$fromdate."          "."TO Date: ".$todate."          "."A/C NO: ".$ac."      "."Report Date: ".date("d-M-Y"); ?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Daily management fee',  messageTop: "<?php echo"From Date: ".$fromdate."          "."TO Date: ".$todate."          "."A/C NO: ".$ac."      "."Report Date: ".date("d-M-Y"); ?>",
