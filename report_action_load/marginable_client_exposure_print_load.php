...
<br>
	...
	<h5 style="float:right;font-size: 12px; font-weight:bold;">Exprosure Statement(Marginable Equity Based on Debt Balance)</h5>
	</div>
    ...
    <tr>
      ...
      <th scope="col">Marginable Equity</th>
      <th scope="col">Market Value of Share</th>
	   <th scope="col">Equity Debt Ratio(%)</th>
  ...
  foreach($ac_type as $margin){
	 $account_no=@$margin['dp_internal_ref_number'];
	 $db_obj->sql("SELECT SUM(acmf_interest_amt) as acmf,SUM(daily_interest) as CashEDF FROM tbl_interest_on_credit_bal WHERE ac_no='$account_no' AND status=0");
     $acmf_fee=$db_obj->getResult();
   if($acmf_fee){
 ...
 $db_obj->sql("SELECT SUM(daily_interest)as total_fee_amt FROM tbl_interest WHERE ac_no='$account_no' AND status=0 AND code='mf'");
 $pm_fee=$db_obj->getResult();
 if($pm_fee){
 ...
 $db_obj->sql("SELECT SUM(daily_interest)as total_fee_amt FROM tbl_interest WHERE ac_no='$account_no' AND status=0 AND code='mi'");
 $loan_fee=$db_obj->getResult();
 if($loan_fee){
	...
	$interest_on_loan=0;
	 ...
	 $db_obj->sql("SELECT SUM(immatured_bal) as im_bal FROM sale_share WHERE account_no='$account_no' AND status=0 Group By account_no");
	 $ress=$db_obj->getResult();
     $immatured_bal=@$ress[0]['im_bal'];
    
  
     $db_obj->sql("SELECT SUM(deposit_amt) as deposit_amt FROM tbl_deposit_balance WHERE account_ref='$account_no' AND status=0 Group By account_ref");
     ...
     $unclear_cheque=@$res_bal[0]['deposit_amt'];
	  ...
	  $db_obj->sql("SELECT account_no,SUM(market_val)as market_val FROM tbl_ipo INNER JOIN instrument ON tbl_ipo.ISIN=instrument.isin WHERE account_no='$account_no' AND non_marginable='FALSE'");
      $resultp=$db_obj->getResult();
	  $ledger_bal=$Avai_balance+$unclear_cheque+$immatured_bal;
	
	if($resultp){
	 $market_val=@$resultp[0]['market_val'];
     $total_equity=$ledger_bal+$market_val-$accrue_charge-$unclear_cheque;
...
var title = 'CAPM Advisory Limited' + '\n' + 'Tower Hamlet(9th floor),' + '\n' + '16 Kamal Ataturk Avenue,Banani C/A'+ '\n' +'Dhaka-1213,Bangladesh.'+ '\n' + 'Exprosure Statement(Marginable Equity Based on Debt Balance)';
	$(document).ready(function() {
    $('#example').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', footer: true, title: title, filename: 'Exprosure StatementMarginable',  messageTop: "<?php echo"To Date: ".$fromdate."          " . "A/C NO: ".$ac_no."      "."Report Date: ".date("d-M-Y"); ?>" },
            { extend: 'pdfHtml5', footer: true, title: title, filename: 'Exprosure StatementMarginable',  messageTop: "<?php echo"From Date: ".$fromdate."          " . "A/C NO: ".$ac_no."      "."Report Date: ".date("d-M-Y"); ?>",
