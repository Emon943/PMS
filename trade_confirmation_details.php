...
if($ac){
 ...
 //print_r($buy_sell);
  ...
  $total=number_format((float)$totals, 2);
		...
		$pdf->Cell(0,10,'Trade Confirmation Details (Customer Copy)',0,0,'C');
		$pdf->Ln(10);
		...
		$pdf->Cell($width_cell[0],10,'Trade',BTL,0,'C',true); // Second header column
		$pdf->Cell($width_cell[1],10,'Instrument',BT,0,'C',true); // Second header column
		$pdf->Cell($width_cell[2],10,'Quantity',BT,0,'C',true); // Fourth header column
		$pdf->Cell($width_cell[3],10,'Rate',BT,0,'C',true); // Fourth header column
		$pdf->Cell($width_cell[4],10,'Comm.',BT,0,'C',true); // Fourth header column
		$pdf->Cell($width_cell[5],10,'Amount',BT,0,'C',true); // Fourth header column
		$pdf->Cell($width_cell[6],10,'Net Amount',BTR,0,'C',true); // Fourth header column
		$pdf->Ln(10);
	...
	if($sub_buy_sell){
	 ...
	 foreach($sub_buy_sell as $sbs){	 
		...
		$pdf->Cell($width_cell[2],10,number_format((int)$sbs['quantity']),0,'C',true); // Fourth column of row 1 
		$pdf->Cell($width_cell[3],10,$sbs['rate'],0,'C',true); // Fourth column of row 1 
		$pdf->Cell($width_cell[4],10,number_format((float)$sbs['commission'],2),0,'C',true); // Fourth column of row 1\
		$pdf->Cell($width_cell[5],10,number_format((float)$sbs['total_amount'],2),0,'C',true); // Fourth column of row 1\
		$pdf->Cell($width_cell[6],10,number_format((float)$sbs['net_amount'],2),0,'C',true); // Fourth column of row 1\
		$pdf->Ln(3);
        $pdf->Write(8, '                          ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------');
        $pdf->Ln(3);  	
 }
		...
		$pdf->Cell($width_cell[2],10,$bqty,0,'C',true); // Fourth column of row 1 
	...
	$ssql_sum="SELECT SUM(quantity) as sqty,SUM(commission) as scomm,SUM(total_amount) as samt,SUM(net_amount) as snamt FROM tread_data_list WHERE account_no='$ac_no' AND `curr_date`='$as_on' AND type='S'";
		...
		$pdf->Cell($width_cell[2],10,number_format((int)$ssbs['quantity']),0,'C',true); // Fourth column of row 1 
		$pdf->Cell($width_cell[3],10,$ssbs['rate'],0,'C',true); // Fourth column of row 1 
		$pdf->Cell($width_cell[4],10,number_format((float)$ssbs['commission'],2),0,'C',true); // Fourth column of row 1\
		$pdf->Cell($width_cell[5],10,number_format((float)$ssbs['total_amount'],2),0,'C',true); // Fourth column of row 1\
		$pdf->Cell($width_cell[6],10,number_format((float)$ssbs['net_amount'],2),0,'C',true); // Fourth column of row 1\
		$pdf->Ln(3);
        $pdf->Write(8, '                    ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------');
        $pdf->Ln(3);		
      }
		...
		$pdf->Cell($width_cell[0],10, '',0,'C',true); // First column of row 1 
		$pdf->Cell($width_cell[1],10, 'Total',0,'C',true); // First column of row 1 
		$pdf->Cell($width_cell[2],10,$sqty,0,'C',true); // Fourth column of row 1 
		$pdf->Cell($width_cell[3],10,'',0,'C',true); // Fourth column of row 1 
