...
//echo $start_date;
 ...
 if($start_date){
   ...
   $startsql="SELECT * FROM tbl_calendar WHERE status=3";
   ...
   $result_date = $dbObj->select($csql);
	...
	$icbdate=$result_date[$j]['date'];
	$icb_date = date('Y-m-d', strtotime($icbdate));
    ...
    VALUES ('$ac_num','$code','$type','$irf','$interest_total_bal','$desc','$daily_creadit_fee','$tax_on','$daily_MB_fee','$icb_date')";
	...
	$daily_management_interest=abs($daily_management_fee);	
	}else{
	    if($Avi_balance > 0){
		 $PMF=($Avi_balance+$market_val);
         $daily_management_f=($PMF*$ir/100);
		 $daily_management_fee=($daily_management_f/360);
		 $daily_management_interest=abs($daily_management_fee);		
		 }else{
	...
	$mfdate=$result_date[$k]['date'];
	$mf_date = date('Y-m-d', strtotime($mfdate));
    $sql="INSERT INTO tbl_interest(ac_no,code,type,interest_rate,amount,description,daily_interest,date)
    VALUES ('$ac_number','$code','$type','$ir','$PMF','$des','$daily_management_interest','$mf_date')";
    ...
    for($i=0; $i<count($interest_margin_ac); $i++){
		 ...
		 $daily_interest=abs($daily_interest_l);
		 for($l=0; $l<count($result_date); $l++){
	     $midate=$result_date[$l]['date'];
	     $mi_date = date('Y-m-d', strtotime($midate));
