<?php 
  ...
  /*-------------------------------------BO Closing Start-----------------------------------*/
   ...
   if($bo=="08DP04UX.TXT"){
	   ...
	   $ai[1] = isset($arrM[1])? $arrM[1] : '';
	   $ai[2] = isset($arrM[2])? $arrM[2] : '';
	   $closing_date = date("Y-m-d", strtotime($ai[2]));
	   $ai[3] = isset($arrM[3])? $arrM[3] : '';
	   $request_date = date("Y-m-d", strtotime($ai[3]));
	   $ai[4] = isset($arrM[4])? $arrM[4] : '';
	   $ai[5] = isset($arrM[5])? $arrM[5] : '';
	  
	 if($ai[0]){
    $sql="INSERT INTO tbl_bo_closing(bo_id,bo_name,closure_date,request_date,reason_for_failure)
    VALUES ('{$ai[0]}','{$ai[1]}','$closing_date','$request_date','{$ai[4]}')";
	//die();
	$res= $dbObj->insert($sql);
   
    $sql = "update investor set status='1' WHERE investor_ac_number='$ai[0]'";
	$dbObj->update($sql);
	}
	 ...
	 <div style="padding-left:20px">
	 <div style="float:left"><img src="pms_doc_upload/investor_cdbl/ok.jpg"width="20" height="20"/></span></div><div style="float:left"><strong><span style="Color:green">BO Closing Successfully...</span></strong></div>
	</div></br>
	 <?php
  }
 ...
 /*  if($bo=="16DP71UX.TXT"){
  ...
  /*---------------------------------------------------------------*/
   ...
   if($bo=="11DP41UX.TXT"){
	   ...
	   $isin = isset($arrM[0])? $arrM[0] : '';
	   $instrument_name = isset($arrM[1])? $arrM[1] : '';
	   $Seq_No = isset($arrM[2])? $arrM[2] : '';
	   $bo_id = isset($arrM[7])? $arrM[7] : '';
	   $qty = isset($arrM[9])? $arrM[9] : '';
	   $ai[12] = isset($arrM[12])? $arrM[12] : '';
	   $date = date("Y-m-d", strtotime($ai[12]));
	   
	  $sqls="SELECT short_name,market_price FROM instrument WHERE isin='$isin'";
      $result= $dbObj->select($sqls);
	 
	  $short_name=@$result[0]['short_name'];
	  $market_price=@$result[0]['market_price'];
	  $market_value=$qty*$market_price;
	  
	  $sql1="SELECT dp_internal_ref_number FROM investor WHERE investor_ac_number='$bo_id'";
      $res= $dbObj->select($sql1);
	  $ac_no=@$res[0]['dp_internal_ref_number'];
	
	  
	if($isin){
     $sql="INSERT INTO tbl_ipo(account_no,ISIN,Seq_No,BO_ID,instrument,Current_Bal,avg_rate,market_val,date)
     VALUES ('$ac_no','$isin','$Seq_No','$bo_id','$short_name','$qty','$market_price','$market_value','$date')";
	 $res= $dbObj->insert($sql);
	 ...
	 <div style="padding-left:20px">
	 <div style="float:left"><img src="pms_doc_upload/investor_cdbl/ok.jpg"width="20" height="20"/></span></div><div style="float:left"><strong><span style="Color:green">Link Share Transferred Successfully...</span></strong></div>
	</div></br>
	 <?php
  }
  ...
  /*------------------------------------------End Link Share--------------------------------------*/
   ...
   if($bo=="16DP95UX.TXT"){
		...
		$ai[2] = isset($arrM[2])? $arrM[2] : '';
	    $ai[3] = isset($arrM[3])? $arrM[3] : '';
	    $eff_date=date("Y-m-d", strtotime($ai[3]));
	    $ai[4] = isset($arrM[4])? $arrM[4] : '';
		$ai[5] = isset($arrM[5])? $arrM[5] : '';
	    $ai[6] = isset($arrM[6])? $arrM[6] : '';
	    $ai[7] = isset($arrM[7])? $arrM[7] : '';
	    ...
	    $ai[11] = isset($arrM[11])? $arrM[11] : '';
		$ai[12] = isset($arrM[12])? $arrM[12] : '';
		$lockin_date=date("Y-m-d", strtotime($ai[12]));
	 ...
	 if($ai[4]){
	  ...
	  $iposql="SELECT * FROM ipo_application WHERE process_status='3' AND bo_id='$ai[4]'";
      $res_ipo_status= $dbObj->select($iposql);
	  $short_name=@$res_ipo_status[0]["short_name"];
	  //die();
	  
	  $iposql="SELECT * FROM tbl_IPO WHERE ISIN='$ai[0]' AND BO_ID='$ai[4]'";
      $res_placement_ipo= $dbObj->select($iposql);
	  //die();
	if($res_placement_ipo){
	 $Current_Bal=@$res_placement_ipo[0]["Current_Bal"];
	 $total_update_bal=$Current_Bal+$free_balance;
	 $sql_ups = "update tbl_ipo set Current_Bal='$total_update_bal' WHERE BO_ID='$ai[4]' AND ISIN='$ai[0]'";
	 $dbObj->update($sql_ups);
	 }else{
     ...
     VALUES ('$ac_no','{$ai[0]}','{$ai[2]}','{$ai[3]}','{$ai[4]}','$short_name','{$ai[6]}','{$ai[7]}','$free_balance','{$ai[9]}','0','{$ai[12]}','$cur_date')";
	 $res= $dbObj->insert($sql); 
	 }
	 $sql_ipo = "update ipo_application set process_status='4',description='IPO Receieve' WHERE bo_id='$ai[4]' AND process_status='3'";
	 $dbObj->update($sql_ipo);
	 
     $sql1="INSERT INTO tbl_dp95(ISIN,ShortName,SequenceNo,EffectiveDate,BOID,BOShortName,BOCategory,BOAccStatus,CurrentBal,LockinBal,ProcFlag,LockExpDate) 
     VALUES ('{$ai[0]}','$short_name','{$ai[2]}','$eff_date','{$ai[4]}','{$ai[5]}','{$ai[6]}','{$ai[7]}','{$ai[8]}','{$ai[9]}','{$ai[10]}','$lockin_date')";
	 $res= $dbObj->insert($sql1); 
	  }
     }?>
	<div style="padding-left:20px">
    <div style="float:left"><img src="pms_doc_upload/investor_cdbl/ok.jpg"width="20" height="20"/></span></div><div><strong><span style="Color:green">IPO Share Update Successfully </span><?php echo $ai[0];?></strong><a href="house_keeping.php?Instrument_update&getID=<?php echo urlencode($enc->encryptIt($short_name));?>" target="_blank"><span class="blink_text" style="Color:red"> Click here for ISIN Update</span></a></div>
	</div></br>
	 <?php
  }
  ...
  if($bo=="17DP48UX.TXT"){
	...
	$content = file_get_contents($path.'/'.$bo);
		...
		$ai[17] = isset($arrM[17])? $arrM[17] : '';
		$ai[18] = isset($arrM[18])? $arrM[18] : '';
		$record_date = date("Y-m-d", strtotime($ai[4]));
	    $payment_date = date("Y-m-d", strtotime($ai[7]));
		
	  
    if($ai[9]){ 
    $sql="INSERT INTO tbl_cash_dividend(bo_id,isin,short_name,record_date,payment_date,cash_fac_amt,tax_amount,bo_holding,net_cash)
    VALUES('{$ai[9]}','{$ai[0]}','{$ai[1]}','$record_date','$payment_date','{$ai[6]}','{$ai[16]}','{$ai[18]}','{$ai[17]}')";
	$res= $dbObj->insert($sql);
	}
     }?>
	<div style="padding-left:20px">
  ...
  if($bo=="17DP64UX.TXT"){
	...
	if($avg_rate==0){
	 $db_obj->sql("SELECT cost_per_share FROM instrument WHERE isin='".$db_obj->EString($ai[0])."'");
     $cost_per_res=$db_obj->getResult();
	
	 $cost_per_share=$cost_per_res[0]['cost_per_share'];
	 $total_amt=$total_share*$cost_per_share;
	}else{
	 $total_amt=$total_share*$avg_rate;
	}
	 
	 
    $sql="INSERT INTO tbl_bonus_share(isin,isin_short_name,share_cat,record_date,effective_date,Cash_Fraction_Amt,bo_id,bo_holding,qty)
    VALUES('{$ai[0]}','{$ai[1]}','{$ai[3]}','$record_date','$payment_date','{$ai[6]}','{$ai[13]}','{$ai[15]}','{$ai[16]}')";
	$res= $dbObj->insert($sql);
	if($ai[3]=='BONUS'){
	$update_avg_cost=@$total_amt/$total_qty;
	$sql_bonus_sql="update tbl_ipo set bonus_share='$update_bonus',avg_rate='$update_avg_cost' WHERE BO_ID='$ai[13]' AND ISIN='$ai[0]'";
	$dbObj->update($sql_bonus_sql);
	}
  ...
  if($bo=="17DP70UX.TXT"){
	...
	foreach($array as $cont){ 
	    ...
	    $ai[15] = isset($arrM[15])? $arrM[15] : '';
	    $ai[16] = isset($arrM[16])? $arrM[16] : '';
		$confirm_date = date("Y-m-d", strtotime($ai[5]));
		$record_date = date("Y-m-d", strtotime($ai[4]));
	  ...
	  $right_share_amt=@$ai[14]*10;
	  $total_share=$Current_Bal+$qty+$bonus_share;
	  $total_amt=$total_share*$avg_rate;
	  $mtotal_amt=$total_amt+$right_share_amt;
	  $mtotal_qty=$total_share+$ai[14];
	  $bonus_update=$bonus_share-$ai[14];
	  $t_curr_bal=$Current_Bal+$ai[14];
	  if($mtotal_qty > 0){
	  $update_avg_cost=@$mtotal_amt/@$mtotal_qty;
	  }
	 if($ai[0]){
	 $csqla="INSERT INTO tbl_corporate_action(isin,instrument,bo_id,name,type,record_date,drcr,qty,locking_bal,freez_bal,confirm_date) 
     VALUES ('{$ai[0]}','{$ai[1]}','{$ai[10]}','{$ai[11]}','{$ai[3]}','$record_date','{$ai[13]}','{$ai[14]}','{$ai[15]}','{$ai[16]}','$confirm_date')";
	 $res= $dbObj->insert($csqla); 
	 
	  if($ai[3]=='BONUS'){
	  $sql = "update tbl_ipo set bonus_share='$bonus_update',Current_Bal='$t_curr_bal' WHERE BO_ID='$ai[10]' AND ISIN='$ai[0]'";
	  $res=$dbObj->update($sql);
	  }else{
	  $sql = "update tbl_ipo set Current_Bal='$t_curr_bal',avg_rate='$update_avg_cost' WHERE BO_ID='$ai[10]' AND ISIN='$ai[0]'";
	  $res=$dbObj->update($sql);
	  }
	 }
     }
