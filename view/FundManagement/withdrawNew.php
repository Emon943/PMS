	...
	if($page['add_status']!="Active"){
        ...
        $wd=$_POST['withdraw_amt'];
		$withdraw_amt=str_replace(',', '', $wd);
		$ck=input_check(array('account_ref'=>$_POST['account_ref']));
        ...
        $db_obj->select('investor','*',"dp_internal_ref_number='".$_POST['account_ref']."'");
        ...
        $_SESSION['in_result_data']= "<h3 align='center'>Withdraw Balance Not Available In Your A/C !<h3>";
          $db_obj->disconnect();
             
			 echo'<script>window.location="fund_management.php?withdrawNew";</script>';
               exit(); 

        }else{
        $withdraw_amt=$_POST['withdraw_amt'];
        $withd_amt=str_replace(',', '', $withdraw_amt);
		$Avi_balance=$_POST['avaiable_bal'];
        $Avi_bal=str_replace(',', '', $Avi_balance);
		
       $ins=$db_obj->insert('tbl_withdraw_balance',array('our_ref'=>$_POST['our_ref'],
		...
		'des'=>$_POST['des'],
		'branch'=>$_POST['branch'],
		'avaiable_bal'=>$Avi_bal,
		'withdraw_amt'=>$withd_amt,
        ...
        echo "<h2 style='text-align:center; color:green'>Withdrawal Insert Successfully</h2>";
...
<script>
  ...
  xhttp.onreadystatechange = function() {
   ...
   testAjax='ajax/withdraw_check.php?text='+account_ref;
     ...
     <?php
          ...
          if(@$_SESSION['in_result_data']){
           ...
           if($page['view_status']=="Active"):
					?>
          <div align="right"><a href="?withdrawal" class="btn alert-info"><span class="icon-th-large"></span> All Withdraw</a></div>
                     <?php endif;?>
   <h4 class="widgettitle">Withdrawal from</h4>
...
<table width="783" border="0" align="center" cellpadding="0" cellspacing="0" >
    <tr>
    <td height="25" colspan="4" align="center" valign="top"  style=  "background-color:#CCC;"><strong>Fund Withdrawal</strong></td>
   </tr>         
  <tr>
  ...
  <tr>
    ...
    <td width="283"><textarea name="des" rows="2" id="des">Fund Withdrawal</textarea></td>
  ...
  <tr>
	...
	<td width="155" height="33"><strong>Withdraw Amt.:</strong></td>
     <td width="283"><input type="text" name="withdraw_amt" id="withdraw_amt" onkeyup="word.innerHTML=convertNumberToWords(this.value)" onblur="withdrawrequest(this.value)" required=""/></td>
...
<script>
	$(document).ready(function(){
		...
		$('#bank_id').change(function(){
			var branch_name = $("#bank_id" ).val();
			 //alert(select);
			$.ajax({
			type:'post',
			url:'ajax/branch_info.php',
