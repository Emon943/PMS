...
if($all){
   $sql="SELECT * FROM `tbl_interest_quaterly` where code='mf' AND `date` BETWEEN '$fromdate' AND '$todate'";
   $res= $dbObj->select($sql);
  // print_r($res);
   $sql1="SELECT SUM(total_interest_amt)as total FROM `tbl_interest_quaterly` where code='mf' AND `date` BETWEEN '$fromdate' AND '$todate'";
...
//die();
 elseif($Panding==0){
   $sql="SELECT * FROM `tbl_interest_quaterly` where status=$Panding AND code='mf' AND `date` BETWEEN '$fromdate' AND '$todate'";
   $res= $dbObj->select($sql);
  // print_r($res);
   $sql1="SELECT SUM(total_interest_amt)as total FROM `tbl_interest_quaterly` where status=$Panding AND code='mf' AND `date` BETWEEN '$fromdate' AND '$todate'";
   $result= $dbObj->select($sql1);
   //print_r($result);
 }
elseif($Posted==1){
   $sql="SELECT * FROM `tbl_interest_quaterly` where code='mf' AND status=$Posted AND `date` BETWEEN '$fromdate' AND '$todate'";
   $res= $dbObj->select($sql);
    $sql1="SELECT SUM(total_interest_amt)as total FROM `tbl_interest_quaterly` where code='mf' AND status=$Posted AND `date` BETWEEN '$fromdate' AND '$todate'";
   $result= $dbObj->select($sql1);
}
else{
   $sql="SELECT * FROM `tbl_interest_quaterly` where code='mf' AND ac_no='$ac_no' AND `date` BETWEEN '$fromdate' AND '$todate'";
   $res= $dbObj->select($sql);
   $sql1="SELECT SUM(total_interest_amt)as total FROM `tbl_interest_quaterly` where code='mf' AND ac_no='$ac_no' AND `date` BETWEEN '$fromdate' AND '$todate'";
   $result= $dbObj->select($sql1);
}
...
VALUES ('$ac_no','mf','Payment','0.00','0.00','$desc','$ab','$ibb_date')";
$ress= $dbObj->insert($sql);
	echo "<h2 align='center' style='color:#000000'>MF Adjustment Successfully</h2>";
    echo "<meta http-equiv='refresh' content='1 URL=fund_management.php?management_fee'>";
    exit;
}
?>
<br/>
...
<br/>
<div align="left"> <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal">Management Fee Adjustment</button></div>
<form id="att_form" name="att_form" method="post" enctype="multipart/form-data" >
 <!-- The Modal -->
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">Management Fee Adjustment</h4>
...
<table width="100%" class="table table-bordered" id="dyntable">
   ...
   <a href="?management_fee_payment" class="btn btn-primary"><span class="icon-plus-sign"></span>Generate</a>
