...
<?php 
	...
	$id=$enc->decryptIt($_GET["getID"]);
		...
		$sql="UPDATE `tbl_deposit_balance` SET `net_balance`='$deposit_amt',`data_insert_id`='$user_id',`status`='1' WHERE `id`='$id'";
        $result1=@mysql_query($sql);
		
	   if($result1){
	   $msg = $account_dp." Code have deposited.Please aprove this code";
       $sub="Deposit Pending List";
	   $email="farzana@capmadvisorybd.com";
		...
		$mc = $mail->send();
		   ...
		   echo "<h2 style='text-align:center; color:green'>Deposite Successfully Confirmed</h2>";
