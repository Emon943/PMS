 ...
 $sql="SELECT ac_no,date,code,type,SUM(daily_interest)as total_interest_amt,description,status FROM `tbl_interest` where code='mf' AND status='0' AND `date` BETWEEN '$fromdate' AND '$todate' GROUP BY ac_no";
  $res= $dbObj->select($sql);
   //print_r($res);
    ...
    VALUES ('$ac_no','$code','$type','$total_interest_amt','$description','$fromdate','$todate','$date')";
	 ...
	 $sql_c="INSERT INTO `tbl_charge_info`(code,ac_no,type,description,company_name,short_name,total_amt,total_balance,date,data_insert_id)
     VALUES('$code','$ac_no','$type','$description','NULL','NULL','$total_interest_amt','$total','$date')";
	 $result= $dbObj->insert($sql_c);
	 
	 $sql2 = "update tbl_interest set status='1' where ac_no ='$ac_no' AND code='mf' AND `date` BETWEEN '$fromdate' AND '$todate'";
     $dbObj->update($sql2);
	 
	 $sql3 = "update investor set total_balance='$total' where dp_internal_ref_number ='$ac_no'";
     $dbObj->update($sql3);
	}
	echo "<h2 align='center' style='color:#000000'>Management Fee Successfully Accrued</h2>";
