...
if($ac_no){
  ...
  $sql="SELECT ac_no,date,code,amount,interest_rate,type,SUM(daily_interest)as total_interest_amt,description,status FROM `tbl_interest` where code='mf' AND status='0' AND `date` BETWEEN '$fromdate' AND '$todate' AND ac_no='$ac_no'";
  $res= $dbObj->select($sql);
}
else{
  
 $sql="SELECT ac_no,date,code,amount,interest_rate,type,SUM(daily_interest)as total_interest_amt,description,status FROM `tbl_interest` where code='mf' AND status='0' AND `date` BETWEEN '$fromdate' AND '$todate' GROUP BY ac_no";
  $res= $dbObj->select($sql);
}
...
<script type="text/javascript">
    function confirmation() {
      return confirm('Are you sure you want to post management fee?');
    }
</script>
<div class="container">
...
<div class="row-fluid">           
   	...
   	<div>
	<form action="?management_accrued" method="post">  
      ...
      <?php endif;?>
          ...
          <thead>
             ...
             <th width="181" class="head1" title="ISIN">Market Value</th>
	 ...
	 <div> &nbsp;&nbsp;Description:<input type="text" name="des" value="<?php echo "Management Fee";?>"></div><br/>&nbsp;&nbsp
