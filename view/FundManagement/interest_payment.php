...
if($ac_no){
  ...
  $sql="SELECT ac_no,date,code,amount,interest_rate,type,SUM(daily_interest)as total_interest_amt,description,status FROM `tbl_interest` where code='mi' AND status='0' AND `date` BETWEEN '$fromdate' AND '$todate' AND ac_no='$ac_no'";
  $res= $dbObj->select($sql);
}
else{
  
  $sql="SELECT ac_no,date,code,amount,interest_rate,type,SUM(daily_interest)as total_interest_amt,description,status FROM `tbl_interest` where code='mi' AND status='0' AND `date` BETWEEN '$fromdate' AND '$todate' GROUP BY ac_no";
  $res= $dbObj->select($sql);
  //print_r($res);
}
...
<div class="row-fluid">           
	...
	<form action="?margin_accrued" method="post">  
	 ...
	 <div> &nbsp;&nbsp;Description:<input type="text" name="des" value="<?php echo "Margin Interest";?>"></div><br/>&nbsp;&nbsp
