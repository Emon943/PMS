<br>
<div>
    <form action="branch_trade_summary.php" target="_blank" method="post" enctype="multipart/form-data" >
   Period:   <label for="pwd">From:</label>
	...
	<select name="Branch" id="Branch" required="required">
               ...
               <?php
                       $bcsql = "select * from branch_list";
                       $db_obj->sql($bcsql);
					   $investor_cat_res=$db_obj->getResult();
						//print_r($investor_cat_res);
                        for ($i = 0; $i < count($investor_cat_res); $i++){ 
                            ?>
                            <option value="<?php echo $investor_cat_res[$i]["branch_id"]; ?>">
                              <?php echo $investor_cat_res[$i]["name"]; ?></option>
                        <?php } ?>
				 </select></br></br>
