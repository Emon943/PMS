...
foreach ($_POST['check'] as $id){
   ...
   /* echo "<pre>";
	...
	$result=$dbObj->insert($sql);
	 ...
	 $db_obj->sql("SELECT fbank_ac_no FROM  tbl_bank_transfer WHERE id='".$db_obj->EString($id_no)."'");
	 $bank_ac_info=$db_obj->getResult();
	 $fbank_ac_no=$bank_ac_info[0]['fbank_ac_no'];
	 
	 $sqlr="INSERT INTO `bank_ledger`(bank_ac_no,code,type,internal_ref,voucher_no,description,balance,date)
     VALUES('$fbank_ac_no','$code','Payment','$internal_ref','0.00','$des','$balance','$date')";
	 $res=$dbObj->insert($sqlr);
