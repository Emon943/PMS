...
if(isset($_GET['dell']) && $page['delete_status']=="Active"){
	...
	$payment_info=$db_obj->getResult();
		...
		$db_obj->sql($sql1);
		$broker_payable_bal=$db_obj->getResult();
		$payable_bal=@$broker_payable_bal[0]['payable'];
		$update_nominal_bal=$payable_bal+$payment_amt;
		$res=$db_obj->update("tbl_broker_hous",array("payable"=>$update_nominal_bal),"trace_id=".$nominal_id."");
        if($res){
        $db_obj->delete("tbl_bank_payment", "id='$id'");
		 }
...
<div class="row-fluid">
   	...
   	<div class="span12">
					...
					?>
      <div align="right">
        <a href="?bank_payment" class="btn btn-primary"><span class="icon-plus-sign"></span>Bank Payment</a>
      ...
      <?php endif;?>
          ...
          <thead>
             ...
             <th width="181" class="head1" title="ISIN">Transaction date</th>
              <th width="394" class="head0" title="Name">Our Ref</th>
			   ...
			   <th width="201" class="head1" title="Receipt Type">Payment Amount</th>
		 ...
		 $sql="SELECT * FROM `tbl_bank_payment` where recon_status='0' ORDER BY id DESC";
               ...
               <tr class="gradeX">
                   ...
                   <td><?php echo $payment_bal['your_ref'];?></td>
					...
					<td><?php echo $payment_bal['total_amt'];?></td>
					<td><?php echo $payment_bal['payment_amt'];?></td>
                  ...
                  <td><?php
					...
					<a href="cashbook.php?Update_ledger&getID=<?php echo urlencode($enc->encryptIt($payment_bal['id']));?>" onclick="return confirmation()" class="btn btn-danger" title="confirm Data">Confirm</a>
