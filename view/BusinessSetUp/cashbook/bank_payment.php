 <?php 
	if($page['add_status']!="Active"){
        ...
        $db_obj->select('tbl_bank_payment','*',"our_ref='".$_POST['our_ref']."'");
        if($db_obj->numRows()!=0){

        $_SESSION['in_result_data']= "<h3 align='center'>Data Already Exist !<h3>";
          $db_obj->disconnect();
             
			 echo'<script>window.location="cashbook.php?bank_payment";</script>';
               exit(); 

        }else{
      $des=$_POST['des']." ".$_POST['account_number'];			
       $ins=$db_obj->insert('tbl_bank_payment',array('trans_date'=>$_POST['date'],
		...
		'receipt_type'=>$_POST['payment_type'],
     ...
     <?php
          ...
          if(@$_SESSION['in_result_data']){
           ...
           if($page['view_status']=="Active"):
					?>
          <div align="right"><a href="?payment_list" class="btn alert-info"><span class="icon-th-large"></span>Payment List</a></div>
                     <?php endif;?>
   <h4 class="widgettitle">House Bank Payment</h4>
 ...
 <form class="stdform" action="" method="post">
  ...
  <tr>
    ...
    <td width="283"><select name="Nominal" id="Nominal" required="required">
    ...
    <?php
	 ...
	 $hqlw="SELECT * FROM `bank_charge_header` where type='Payment'";
	 $db_obj->sql($hqlw);
	 $bank_ch=$db_obj->getResult();
	 $bbc=(array_merge($broker_house,$bank_ch));
						
	foreach($bbc as $broker){
	?>
	
    <option value="<?php echo $broker['trace_id'];?>"><?php echo $broker['trace_id']."-".$broker['Internal_code']." ". "Payable"; ?></option>
	 <?php
	}
	?> 	
    </select></td>
...
<script>
	...
	$(document).ready(function(){
		...
		$('#Nominal').change(function(){
			var select = $("#Nominal" ).val();
			 //alert(select);
			$.ajax({
		type:'post',
		url:'ajax/nominal.php',
		data:'s_text='+select,
		success: function (data) {
         $('#res').html(data);
        }
