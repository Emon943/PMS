            ...
            if(move_uploaded_file($temp_name, $location.$name)){
	           ...
	           //print_r($content);
				...
				//die();
				 $id =@$arrM[2];
				 $short_name =@$arrM[6];
				 $allotted_share =@$arrM[10];
				 $refund_bal =@$arrM[12];
				 if($id){
				 $sql="UPDATE ipo_application SET `process_status`='1',`refund_amt`='$refund_bal',`market_lot`='$allotted_share',`description`='Pandding for IPO' WHERE `ac_no`='$id' AND short_name='$short_name'";
...
<div>
 <h4 class="widgettitle">Search By Company</h4>
...
<table class="table table-bordered table-striped mb-0">
   ...
   <?php
	...
	?>
		...
		<input type="file" name="file" value="fileupload" id="file" required > <label for="fileupload"> Import Result file</label> 
