        ...
        if(!empty($name)){      
            ...
            if(move_uploaded_file($temp_name, $location.$name)){
			   ...
			   //die();
				...
				$sql_isin="SELECT isin FROM instrument WHERE isin='$ISIN'";
                $res_isin= $dbObj->select($sql_isin);
				 if($res_isin){
				  $sql="UPDATE `instrument` SET `market_price`='$Close',`ins_cat_id`='$Cate' WHERE `short_name`='$short_name'";
                  $result=@mysql_query($sql);
				 }else{
				  $sql="UPDATE `instrument` SET `market_price`='$Close',`isin`='$ISIN',`ins_cat_id`='$Cate' WHERE `short_name`='$short_name'";
                  $result=@mysql_query($sql); 
				 }
