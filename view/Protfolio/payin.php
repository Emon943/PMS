...
<div class="row-fluid">
      ...
      <?php endif;?>
          ...
          <h2 style="text-align:center">Transfer of Securities(PayIn) Transfer DP Reports</h2></br>             
          ...
          <tbody>
                   ...
                   <?php
					...
					$sql="SELECT * FROM tbl_payin";
					
					
					 $db_obj->sql($sql);
					 $payin_r=$db_obj->getResult();
					// print_r($daily_bo_holding);
					 //die();
					if($payin_r){
						
						foreach($payin_r as $payin):
						
					?>  
              <tr class="gradeX">
                 <td><?php echo $payin['sl_no'];?></td>
                 <td><?php echo $payin['dp_code'];?></td>
				 <td><?php echo $payin['name'];?></td>
				 <td><?php echo $payin['bo_id'];?></td>
				 <td><?php echo $payin['instument'];?></td>
				 <td><?php echo $payin['quantity'];?></td>
