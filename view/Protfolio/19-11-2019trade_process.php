...
$total_buy="SELECT date_tread,bokersoftcode,branch_id, SUM(total_amount) as total_buy_amt, SUM(broker_commission) as total_buy_bcharges, SUM(agency_comm) as total_buy_agency_commi, SUM(commission) as total_buy_commission FROM tread_data_list WHERE date_tread='$date_tread' AND status=0 AND type='B' Group By type";
$result_total_buy= $dbObj->select($total_buy);
 ...
 $bokersoftcode=$result_total_buy[0]["bokersoftcode"];
 ...
 $total_sell="SELECT date_tread,bokersoftcode,branch_id,SUM(total_amount) as total_sell_amt, SUM(broker_commission) as total_sell_bcharges, SUM(agency_comm) as total_sell_agency_commi, SUM(commission) as total_sell_commission FROM tread_data_list WHERE date_tread='$date_tread' AND status=0 AND type='S' Group By type";
 $result_total_sell= $dbObj->select($total_sell);
 if($result_total_sell){
 $tread_date=@$result_total_sell[0]['date_tread'];
 $tread_date = date('m-d-Y', strtotime($tread_date));
 $total_sell_amt=@$result_total_sell[0]['total_sell_amt'];
 $bokersoftcode=$result_total_sell[0]["bokersoftcode"];
 ...
 //$grand_total_sell_commi=$total_sell_bcharges+$total_sell_commission+$total_sell_agency_commi;
  ...
  echo $sql_pay_rec="SELECT * FROM `tbl_broker_hous` trace_id='$bokersoftcode'";
  $result_pay_rec= $dbObj->select($sql_pay_rec);
  print_r($result_pay_rec);
  echo $payable_bal=@$result_pay_rec[0]['payable'];
  ...
  if($broker_payable > $broker_receivable){
    ...
    $sqlpr = "update tbl_broker_hous set payable='$pay',receivable='$rece' where trace_id='$bokersoftcode'";
    $dbObj->update($sqlpr);
	/*End Payable Receivable Broker House*/
  
  $sqlbs = "insert into tbl_buy_sell_info(trade_date,branch_id,broker_code,total_buy,total_sell,total_turn_over,broker_buy_commi,broker_sell_commi,total_commission,broker_payable,broker_receivable)
		values('$tread_date','$branch_id','$bokersoftcode','$total_buy_amt','$total_sell_amt','$total_turn_over','$total_buy_bcharges','$total_sell_bcharges','$total_commission','$broker_payable','$broker_receivable')";
   ...
   if($bo_id && $isin){
    ...
    $resi= $dbObj->select($sqli);
        ...
        $sqlss = "insert into sale_share
				...
				values(
				'$ac_no','$bo_id','$bokersoftcode','$dsc_clear_bo','$instrument','$isin','$quantity','$avg_rate','$commission','$immatured_bal','$category','$date_tread','$settlement_day')";
				//die();
...
$sql="SELECT account_no,bo_id,date_tread,category,COUNT(id) as num, instrument, isin, SUM(quantity) as qty, SUM(net_amount)as gt, SUM(rate)as avg, SUM(commission)as commi FROM tread_data_list WHERE date_tread='$date_tread' AND status=0 AND type='B' Group By account_no,isin";
$result= $dbObj->select($sql);
 ...
 for ($i = 0; $i < count($result); $i++) {
  ...
  $sql = "insert into tbl_ipo
	...
	$sql_buy_in = "insert into tbl_buy_info
     ...
     VALUES('B','$buy_ac_no','Buy','$instrument','$instrument','NULL','$buy_quantity','$buy_avg_rate','$buy_gt','$commi','0.00','$bdate')";
 ...
 /* settlement process start*/
  ...
  $sqlbuying="SELECT account_no,SUM(net_amount)as gt FROM tread_data_list WHERE date_tread='$date_tread' AND status='0' AND type='B' AND category!='Z' Group By account_no";
  ...
  for($i=0; $i<=count($result_buy_info); $i++){
	...
	$sql_im="SELECT id,SUM(immatured_bal) as immatured_bal,account_no FROM sale_share WHERE account_no='$acc_no' AND status='0' AND trade_date='$date_tread' AND cate!='Z' Group By account_no";
	...
	$account_no =@$immatured_bal_check[0]["account_no"];
	  ...
	  $sql_id="SELECT id,immatured_bal FROM sale_share WHERE account_no='$account_no' AND settlement_status='0' AND cate!='Z'";
