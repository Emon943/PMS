        ...
        ON (`investor_exchange_details`.`investor_id` = `investor`.`investor_id`) WHERE `investor`.`investor_id`='".$db_obj->EString($investorID)."'");
			  ...
			  if(!$investor){
				   echo "<h2>Investor Not Founded...........</h2>";
					...
					$employee_id=$investor[0]["employee_relation_id"];
...
<div class="row-fluid" id="mydiv">
     ...
     <?php
				...
				if(isset($_POST["updateInv"])){
					...
					@move_uploaded_file($SigniUP['tmp_name'], $investor_SigniUP);
					  ...
					  $curr_date = date('Y-m-d H:i:s');
					   $ins=$db_obj->insert('investor_group_log',array('client_code'=>$_POST['dp_internal_ref_number'],
					  ...
					  $db_obj->update('investor',array('investor_name'=>$_POST["investor_name"],'account_status'=>$_POST["account_status"],
         					 'dp_internal_ref_number'=>$_POST["dp_internal_ref_number"],'investor_ac_number'=>$_POST["investor_ac_number"],'status'=>$_POST["status"],
							 'investor_group_id'=>$_POST["investor_group_id"],'managment_fee_id'=>$_POST["managment_fee_id"],
							 'interest_on_credit_blance_cata_id'=>$_POST["interest_on_credit_blance_cata_id"],'agency_id'=>$_POST["agency_id"],'created_set_date'=>$curr_date,'employee_relation_id'=>$_POST["employee_id"],
					   ...
					   $up=$db_obj->update("investor_personal",array("dp_internal_ref_number"=>$_POST["dp_internal_ref_number"],
							 "bo_id"=>$_POST["investor_ac_number"], "bo_type"=>$_POST["bo_type"],  "bo_catagory"=>$_POST["cate_id"],
          ...
          <table width="80%" border="0" cellpadding="0" cellspacing="5">
            ...
            <tr>
              ...
              <td align="center"><strong>
                <input name="investor_ac_number" type="text" id="investor_ac_number" value="<?php echo $investor[0]['investor_ac_number']?>"/>
              </strong></td>
            </tr>
            <tr>
           ...
           <tr>
            ...
            <tr>
			 ...
			 <tr>
              ...
              <td align="center">
			   <select name="cate_id" id="cate_id" required="required">
			   <option value=" "></option>
               <?php
					   ...
					   $investor_cat_res=$db_obj->getResult();
                        ...
                        for ($i = 0; $i < count($investor_cat_res); $i++) {
                            if(@$cate_id == $investor_cat_res[$i]["id"]){
                                $bo_catagory = "selected";
                            }else{
                                $bo_catagory = "";
								}
                            ?>
                            <option <?php echo $bo_catagory; ?> value="<?php echo $investor_cat_res[$i]["id"]; ?>">
      ...
      <?php
			...
			<tr>
              <td><strong>Employee Relation</strong></td>
              <td align="center"><select name="employee_id" id="employee_id" required="required">
			   <option value=" "></option>
			  <?php
                       $empsql = "select * from employee order by id";
                       $db_obj->sql($empsql);
					   $relation_setup=$db_obj->getResult();
						//print_r($investor_group_res);
                        for ($i = 0; $i < count($relation_setup); $i++){
                            if ($employee_id == $relation_setup[$i]["id"]){
                                $emp_name = "selected";
                            }else{
                                $emp_name = "";
								}
                            ?>
                            <option <?php echo $emp_name; ?> value="<?php echo $relation_setup[$i]["id"]; ?>">
                                <?php echo $relation_setup[$i]["login_id"]; ?></option>
                        <?php } ?>
