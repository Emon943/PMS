...
if($emp_id){
  ...
  $sql="SELECT tbl_withdraw_balance.date,tbl_withdraw_balance.account_ref,tbl_withdraw_balance.investor_name,tbl_withdraw_balance.code,tbl_withdraw_balance.our_ref,tbl_withdraw_balance.cheque,tbl_withdraw_balance.voucher_Ref,tbl_withdraw_balance.bank,tbl_withdraw_balance.branch,tbl_withdraw_balance.withdraw_amt,tbl_withdraw_balance.data_insert_id FROM investor INNER JOIN tbl_withdraw_balance ON investor.dp_internal_ref_number=tbl_withdraw_balance.account_ref WHERE employee_relation_id='$emp_id' AND `date` BETWEEN '$fromdate' AND '$todate' AND tbl_withdraw_balance.status=2";
  $result=$dbObj->select($sql);
  }else{
	$sql="SELECT * FROM tbl_withdraw_balance WHERE `date` BETWEEN '$fromdate' AND '$todate' AND status=2";
    $result= $dbObj->select($sql);
	 $emp_name="All";
  }
		...
		$pdf->Cell(0,10,'Withdraw List',0,0,'C');
		$pdf->Ln(10);
		...
		$width_cell=array(15,20,35,25,20,30,25,20);
		...
		//$pdf->Cell($width_cell[5],10,'Cheque Ref',1,0,'C',true); // Fourth header column
		$pdf->Cell($width_cell[5],10,'Bank Name',1,0,'C',true); // Fourth header column
	...
	if($result){
		...
		$total_withdraw=number_format((float)$sum, 2);
	   ...
	   $user_name=$user_res[0]['login_id'];
		...
		//$pdf->Cell($width_cell[5],10,$cheque,0,0,'C',true); // Fourth column of row 1\
		...
		$pdf->Cell($width_cell[6],10,$withdraw_amt_v,0,0,'C',true); // Fourth column of row 1\
		...
		//$pdf->Ln(1); 
        $pdf->SetX(160);	
        $pdf->MultiCell(15,5,$total_withdraw,B,C,false);
		$pdf->Ln(12); 
...
$fileName ='withdraw_list_'.$emp_name.'.pdf';
